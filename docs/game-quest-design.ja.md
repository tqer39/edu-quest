...
**目的**: L 字にジャンプする守護者を配置しながら、空間認識と先読み力を鍛える。

**ゲームの流れ**:

1. 色分けされた領域と禁止マスが散りばめられた 6×6 グリッドを表示する。
2. マスをタップして「守護者」「メモ」「空白」を切り替え、ナイトの動きを意識しながら推理する。
3. 各行・各列・各領域に 1 体ずつ守護者を配置し、互いにナイトの動きで到達できない状態になればクリア。

**特徴**:

- ナイト同士が衝突すると違反マスをハイライトし、原因をメッセージで知らせる。
- 領域ごとの配置制限によって、全体と局所を行き来する論理的思考を促す。
- 行・列・領域の不足を順番にガイドする段階的ヒントを用意し、自力での発見をサポートする。
